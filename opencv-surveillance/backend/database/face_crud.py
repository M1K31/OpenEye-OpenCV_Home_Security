...
def create_recording_event(
    db: Session, camera_id: str, recording_path: str, started_at: datetime
...
def update_recording_event(
    ...
    event = (
        ...
        .filter(models.RecordingEvent.id == event_id)
        .first()
    )
...
def get_system_logs(
    ...
    return query.order_by(desc(models.SystemLog.created_at)).limit(limit).all()


def cleanup_old_events(db: Session, days_to_keep: int = 30) -> Dict[str, int]:
