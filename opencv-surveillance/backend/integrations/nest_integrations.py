...
class NestDevice:
    ...
    def is_camera(self) -> bool:
        ...
        return (
            "sdm.devices.types.CAMERA" in self.type
            or "sdm.devices.types.DOORBELL" in self.type
        )
...
class NestIntegration:
    ...
    async def subscribe_to_events(self, pubsub_topic: str):
        ...
        subscription_path = subscriber.subscription_path(
            self.project_id, "opencv-surveillance-events"
        )
...
async def main():
    ...
    if camera_devices:
        ...
        if image_bytes:
            # Save snapshot
            with open(
                f"nest_snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg", "wb"
            ) as f:
                f.write(image_bytes)
