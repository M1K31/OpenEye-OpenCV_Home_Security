...
class S3Storage:
    ...
    def __init__(self, config: StorageConfig):
        ...
        if config.credentials:
            session_kwargs["aws_access_key_id"] = config.credentials.get(
                "access_key_id"
            )
            session_kwargs["aws_secret_access_key"] = config.credentials.get(
                "secret_access_key"
            )
    ...
    def upload_file(self, local_path: str, remote_path: str,
        ...
        try:
            ...
            logger.info(
                f"Uploaded {local_path} to s3://{self.config.bucket_name}/{key}"
            )
            return True
...
class GoogleCloudStorage:
    ...
    def upload_file(self, local_path: str, remote_path: str,
        ...
        try:
            ...
            logger.info(
                f"Uploaded {local_path} to gs://{self.config.bucket_name}/{blob_name}"
            )
            return True
...
class CloudStorageManager:
            ...
            try:
                ...
                if self.primary_provider:
                    ...
                    success = provider.upload_file(
                        task.local_path, task.remote_path, task.metadata
                    )
    ...
    def delete_file(
        self, remote_path: str, provider_name: Optional[str] = None
    ) -> bool:
    ...
    def cleanup_old_files(
        self, days: int = 30, provider_name: Optional[str] = None
    ) -> int:
        """
