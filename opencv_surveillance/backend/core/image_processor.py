...
class ImageProcessor:
    ...
    SHARPNESS_KERNELS = {
        "none": None,
        "low": np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]], dtype=np.float32),
        "medium": np.array([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]], dtype=np.float32),
        "high": np.array([[-1, -2, -1], [-2, 13, -2], [-1, -2, -1]], dtype=np.float32),
    }
    ...
    def __init__(
        ...
        self.sharpness_kernel = self.SHARPNESS_KERNELS.get(
            self.sharpness, self.SHARPNESS_KERNELS["none"]
        )
    ...
    def has_adjustments(self) -> bool:
        ...
        return (
            ...
            or self.sharpness != "none"
            or self.noise_reduction_strength > 0
        )
