    ...
    def get_snapshot(self, width: int = 1920, height: int = 1080) -> bytes:
        ...
        try:
            ...
            _, buffer = cv2.imencode(
                ".jpg", frame_resized, [cv2.IMWRITE_JPEG_QUALITY, 90]
            )
...
class HomeKitDoorbell(HomeKitCamera):
    ...
    def add_doorbell_service(self):
        ...
        self.doorbell_event_char = doorbell_service.get_characteristic(
...
class HomeKitBridge(Bridge):
    ...
    def add_camera(
        ...
        """
        if is_doorbell:
            camera = HomeKitDoorbell(self.driver, camera_name, camera_source)
        else:
            camera = HomeKitCamera(self.driver, camera_name, camera_source)
...
class HomeKitIntegration:
    ...
    def __init__(
        ...
        self.driver = AccessoryDriver(
            port=port, persist_file=persist_file, pincode=pincode.encode()
        )
    ...
    def get_snapshot(
        self, camera_id: str, width: int = 1920, height: int = 1080
    ) -> bytes:
...
if __name__ == "__main__":
    ...
    homekit = HomeKitIntegration(
        bridge_name="OpenCV Surveillance", pincode="123-45-678"
    )
