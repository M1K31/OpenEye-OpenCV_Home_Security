...
class MQTTIntegration:
    ...
    def publish_recording_event(
        self, camera_id: str, recording: bool, filename: str = None
    ):
        """Publish recording start/stop event"""
        data = {"recording": recording, "filename": filename}
    ...
    def publish_snapshot(self,
        ...
        if metadata:
            meta_topic = (
                f"{self.config.base_topic}/camera/{camera_id}/snapshot/metadata"
            )
    ...
    if mqtt_integration.connect():
        ...
        try:
            while True:
                ...
                mqtt_integration.publish_motion_detected("camera_1", False)
