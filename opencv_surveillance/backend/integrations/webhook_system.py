...
class WebhookManager:
    ...
    def _load_webhooks(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.webhooks)} webhooks from {self.database_file}"
            )
    ...
    def _should_trigger(
        self, webhook: WebhookConfig, event_type: str, camera_id: str
    ) -> bool:
        """
        ...
        if (
            WebhookEvent.ALL.value not in webhook.events
            and event_type not in webhook.events
        ):
            return False
    ...
    async def trigger_event(
        self, event_type: str, camera_id: str, data: Dict[str, Any]
    ):
        """
        ...
        if not matching_webhooks:
            logger.debug(
                f"No webhooks registered for event {event_type} on {camera_id}"
            )
            return
    ...
    async def trigger_motion_detected(
        self, camera_id: str, confidence: float, zones: List[str] = None
    ):
        """Convenience method for motion detection event"""
        await self.trigger_event(
            WebhookEvent.MOTION_DETECTED.value,
            camera_id,
            {"confidence": confidence, "zones": zones or []},
        )
...
async def main():
    ...
    await manager.trigger_motion_detected(
        camera_id="camera_1", confidence=0.95, zones=["zone_1"]
    )
